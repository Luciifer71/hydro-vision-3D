...
export default function DepthPage() {
  ...
  const metrics = useMemo(() => {
    ...
    const list = hazards.map((h, i) => {
      ...
      return {
        id: h.hazard_id || `HAZ-${String(i + 1).padStart(4, '0')}`,
  ...
  const chartData = {
    labels: metrics.list.slice(0, 12).map(x => x.id),
    datasets: [
      {
        label: 'Max Monocular Depth (cm)',
        data: metrics.list.slice(0, 12).map(x => Number(x.depthCm.toFixed(1))),
        backgroundColor: metrics.list.slice(0, 12).map(x => x.depthCm > 15 ? '#ef4444' : x.depthCm > 8 ? '#f59e0b' : '#10b981'),
        borderRadius: 4,
      }
    ],
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div className="info-grid">
        ...
        <div className="info-card" style={{ borderColor: 'rgba(255, 187, 0, 0.4)' }}>
        ...
        <div className="info-card">
          ...
          <h4 style={{ fontSize: '0.88rem', marginBottom: 6 }}>Surface Normals</h4>
          <p style={{ fontSize: '0.78rem', color: 'var(--text-muted)', lineHeight: 1.6 }}>
        ...
        <div className="info-card">
          <span className="coming-soon">Coming Soon</span>
          <h4 style={{ fontSize: '0.88rem', marginBottom: 6 }}>Point Cloud</h4>
          <p style={{ fontSize: '0.78rem', color: 'var(--text-muted)', lineHeight: 1.6 }}>
      ...
      <div className="card">
        <div className="card-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <span className="card-badge badge-live">● Active Dense Inference</span>
        </div>
